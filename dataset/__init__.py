...
from .medical_dataset_dpo import generation_train_dpo, generation_eval_dpo
...
def create_dataset(dataset, tokenizer, args):
    ...
    elif dataset == 'generation_mimic_cxr_dpo':
        print('geneartion_dpo!!!!!')
        train_dataset_dpo = generation_train_dpo(transform_train, args.image_dir, args.ann_path, tokenizer, dataset='mimic_cxr',
                                         args=args)
        val_dataset_dpo = generation_eval_dpo(transform_test, args.image_dir, args.ann_path, tokenizer, split='val',
                                      dataset='mimic_cxr', args=args)
        test_dataset_dpo = generation_eval_dpo(transform_test, args.image_dir, args.ann_path, tokenizer, split='test',
                                       dataset='mimic_cxr', args=args)
        return train_dataset_dpo, val_dataset_dpo, test_dataset_dpo
...
def create_dataset_train(dataset, tokenizer, args):
    transform_test = transforms.Compose([
        transforms.Resize(256),
        transforms.CenterCrop(args.image_size),
        transforms.ToTensor(),
    ...
    if dataset =='generation_iu_xray':
        test_dataset = generation_eval(transform_test, args.image_dir, args.ann_path, tokenizer, split='train', dataset='iu_xray', args=args)
        return test_dataset
    elif dataset =='generation_mimic_cxr':
        test_dataset = generation_eval(transform_test, args.image_dir, args.ann_path, tokenizer, split='train', dataset='mimic_cxr', args=args)
        return test_dataset
