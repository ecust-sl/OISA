...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        wandb.init(
            project="MODPO",  # 项目名称
            name=f"{today_date}_{self.args.mrg_beta}_{self.args.dpo_beta}_{self.args.beta}_{self.args.seed}_{self.w}_{1 - self.w}_cliq" ,  # 运行名称，可自定义
        )
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.is_main_process:
                if log[self.mnt_metric] + log['test_ce_f1'] + log['test_rate_score'] >= self.mnt_best:
                    self.mnt_best = log[self.mnt_metric] + log['test_ce_f1'] + log['test_rate_score']
                    self.log_best = log
